...
1. Add your cluster nodes to `/etc/hosts` on the machine running the script:
   ...
   192.168.1.11    pi-worker1
   192.168.1.12    pi-worker2
   192.168.1.13    pi-worker3
   ```

2. Make the script executable:
   ```bash
   chmod +x cluster-ssh-setup.sh
   ```

3. Run the script:
   ```bash
   ./cluster-ssh-setup.sh
   ```
...
Updating /etc/hosts on pi-worker1...
...
=== Phase 5: Testing mesh connectivity ===
pi-master -> pi-master: OK
pi-master -> pi-worker1: OK
...
=== Done ===
```
